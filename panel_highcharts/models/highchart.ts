...
export class HighChart extends HTMLBox {
    ...
    static init_HighChart(): void {
      ...
      this.define<HighChart.Props>(({ Any }) => ({
        config:             [ Any                              ],
        config_update:             [ Any                              ],
        event:              [ Any                              ],
      }))

      this.override<HighChart.Props>({
        height: 400,
        width: 600
      });
    }
...
function updateJS(config: any, model: HighChart){
  if (config===null){
    return config
  }
...
function sendEvent(event: any, model: HighChart, key: any=null): void {
